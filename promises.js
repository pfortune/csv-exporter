...
async function getOrders() {
	...
	orderIds.forEach((order) => {
		orders.push(getOrderDetails(order.id));
	});

	return Promise.all(orders).then((order) => console.log(order));
}
...
function assignCollectionOrders(order) {
	...
	if (order.id_carrier === COLLECTION) {
		order.address1 = order.address2 = order.city = order.company = order.state = 'Collection';
	}
